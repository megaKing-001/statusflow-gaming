...
import { PvPMatchmaking } from '@/components/football/PvPMatchmaking';
import { Trophy, Target, Coins, Play, RefreshCw, Swords } from 'lucide-react';
...
export function FootballClient({ currentUserId, initialBalance, recentMatches }: FootballClientProps) {
  const [activeTab, setActiveTab] = useState<'penalty' | 'ai_match' | 'pvp'>('penalty');
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 text-slate-100">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-400 text-xs max-w-md mx-auto">
          Stake SFP in penalty shootouts, AI match simulations, or live player-vs-player challenges.
        </p>
      </div>
      ...
      <div className="flex bg-slate-900 p-1.5 rounded-2xl border border-slate-800 max-w-md mx-auto">
        ...
        >
          <Swords size={15} /> PvP Lobbies
        </button>
      </div>

      {/* PVP TAB */}
      {activeTab === 'pvp' && (
        <PvPMatchmaking currentUserId={currentUserId} />
      )}

      {/* SINGLE PLAYER MODES (Penalty & AI Match) */}
      {activeTab !== 'pvp' && (
